...
.cariveau.2016 <- function(...){
  ...
  metadata <- data[,c(1:3,6,15:16)]
  data$species <- with(data, tolower(paste(genus, species, sep="_")))
  data <- data[-c(1:4,6, 15:16)]
  data <- .df.melt(data, "species", units=c(rep(NA, 4), rep("mm", 4)), metadata)
  return(data)
}
...
.simmons.2014 <- function(...){
  ...
  metadata <- data[,12]
  data <- data[,-c(11:12)]
  units <- c(rep(NA,2), "mg", "mm", "mg", rep("log_mg", 2), rep("log_mm", 2))
  data <- .df.melt(data, "species", units, metadata)
  return(data)
}
...
.grootemaat.2015 <- function(...){
    ...
    metadata <- data[,c(1,3)]
    data <- data[,-c(1,3)]
    units <- c(rep('mm',4), rep('g',2), rep('cm^2',2), 'cm^3', '1/cm', rep('g/cm^3',2), "%", "cm^2/g", rep("%",4), 'NA', rep('s',4))
    data <- .df.melt(data, "species", units, metadata)
    return(data)
}
...
.arnold.2017 <- function(...){
  ...
  units <- c("μl_CO2/h", "#/h", "mg", rep("mm",7), rep("mm/s",2), rep(NA,3))
  data <- .df.melt(data, "species", units, metadata)
  return(data)
}
...
.buzzard.2015 <- function(...){
  ...
  units <- c("cm^2/g", NA, "mg/g", rep(NA,3))
  data <- .df.melt(data, "species", units, metadata)
  return(data)
}
...
.limpens.2013a <- function(...){
  ...
  metadata <- data[,c(2:4)]
  data <- data[,-c(2:4)]
  units <- c(rep("cm",2), "mm day^-1", "mg", rep("cm",2), "mm day^-1", "mg", "cm/cm", "mg/mg", "mg")
  data <- .df.melt(data, "species", units, metadata)
  return(data)
}
...
.limpens.2013b <- function(...){
  ...
  metadata <- data[,c(2,4)]
  data <- data[,-c(2,4)]
  units <- c("%", "mg", "cm", rep("mm",2), "cm", "mg", "mm", "cm/cm", NA, "mg", NA, "?")
  data <- .df.melt(data, "species", units, metadata)
  return(data)
}
...
.plourde.2015 <- function(..){
  ...
  units <- c("g", "cm^3", rep("g",2), "cm^3", rep("g",2), "cm^3", "g", "cm", rep("cm", 7), rep("g/cm^3",3), rep("cm^2", 4),  "?", "?", "%")
  data <- .df.melt(data, "species", units, metadata)
  return(data) 
}
...
.jennings.2016a <- function(...){
  ...
  metadata <- data[,c(2:6)]
  data <- data[,-c(2:6)]
  units <- c("#", "sqrt(#)", "cm^2", "cm", "#", "#/cm^2", "cm^2","cm^2")
  data <- .df.melt(data, "species", units, metadata)
  return(data)
}
...
.jennings.2016c <- function(...){
  ...
  metadata <- data[,c(2:6)]
  data <- data[,-c(2:6)]
  units <- c("cm^2", "cm", "#", "sqrt(#)", "g",  "g", "#", "#/cm^2", "cm^2","cm^2")
  data <- .df.melt(data, "species", units, metadata)
  return(data)
}
...
.kraft.2015a <- function(...){
  ...
  metadata <- data[,c(2:4)]
  data <- data[,-c(2:4)]
  units <- c(rep("J/m^2",4), "g", "µm", "µm", "#", "m", "density", "#")
...
.kraft.2015b <- function(...){
  ...
  metadata <- data[,c(2:3)]
  data <- data[,-c(2:3)]
  units <- c(rep("mm",3), "mm^3", rep("mm",3), rep("J/m^2",2), "%", "%", "#", "m", "#", "density")
  data <- .df.melt(data, "Species", units, metadata)
  return(data)
}
...
.bengtsson.2016 <- function(...){
  ...
  units <- c(NA, rep("%",4), "g", "g/cm^2", "g", "g/cm^2", "g", "g/cm^2", "mm", "mm", "cm^-2", "mg/cm^3", "mm", "cm^-2", "mg/cm^3", "mg cm-^2 h^-1", "mg g^-1 h^-1", "mg h^-1", "%", "%", "C/N", "cm^-2", "mg/cm^3", "mm")
